...
//}(unsafeWindow.MT || {}, unsafeWindow.jQuery));

//unsafeWindow.MT = cloneInto(MT, unsafeWindow
//var MT = {};
//var hi = function() {alert('hi');}
//unsafeWindow.MT = cloneInto(MT, unsafeWindow); //will not clone methods
////var MT = createObjectIn(unsafeWindow, {defineAs: "MT"});
//exportFunction(hi, MT, {defineAs: "hi"});


  unsafeWindow.$('#filter').customAutocomplete();
  /*
        unsafeWindow.$('#filter').customAutocomplete({
            textbox: '#text-filter',
            inputList: '.visual .filter-group:not(.keyword)',
